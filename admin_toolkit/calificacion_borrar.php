...
<?php
if ((isset($_POST['idcalificacion'])) && ($_POST['idcalificacion'] != "")) {
  $deleteSQL = sprintf("DELETE FROM dlg_calificacion WHERE idcalificacion=%s",
                       GetSQLValueString($_POST['idcalificacion'], "int"));

  mysql_select_db($database_config, $config);
  $Result1 = mysql_query($deleteSQL, $config) or die(mysql_error());

  $deleteGoTo = "calificacion.php";
...
if (isset($_GET['idcalificacion'])) {
  $colname_registro = $_GET['idcalificacion'];
}
mysql_select_db($database_config, $config);
$query_registro = sprintf("SELECT idcalificacion, calificacion FROM dlg_calificacion WHERE idcalificacion = %s", GetSQLValueString($colname_registro, "int"));
...
<div class="uno"><a class="fa fa-wrench"></a> BORRAR CALIFICACION</div>
<div class="entero">
<form method="POST" action="<?php echo $editFormAction; ?>" name="formguardar" id="formguardar">
  <table width="100%" border="0" cellspacing="0" cellpadding="4">
    <tr>
    <td colspan="4" align="center">Se borrará la calificación: <br><?php echo $row_registro['calificacion']; ?> <input name="idcalificacion" type="hidden" id="idcalificacion" value="<?php echo $row_registro['idcalificacion']; ?>"></td>
    </tr>
      <tr>
