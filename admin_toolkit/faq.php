...
if ((isset($_POST["MM_update"])) && ($_POST["MM_update"] == "formguardar")) {
  $orden = $_POST['orden'];
  $idfaq = $_POST['idfaq'];
  for($i=0;$i<count($idfaq);$i++){
	  $updateSQL = sprintf("UPDATE dlg_faq SET orden=%s WHERE idfaq=%s",
						   GetSQLValueString($orden[$i], "int"),
						   GetSQLValueString($idfaq[$i], "int"));
...
if (isset($_GET['pregunta'])) {
  $colname_registros = $_GET['pregunta'];
}
mysql_select_db($database_config, $config);
$query_registros = sprintf("SELECT idfaq, pregunta, orden FROM dlg_faq WHERE pregunta LIKE %s ORDER BY orden ASC", GetSQLValueString("%" . $colname_registros . "%", "text"));
...
<div class="uno"><a class="fa fa-wrench"></a> FAQ</div>
<div class="entero">
<table width="100%" border="0" cellspacing="0" cellpadding="4">
  <tr>
    <td colspan="3">
    <form action="faq.php" method="get" id="formbuscar" name="formbuscar">
    <strong>Pregunta
      <input name="pregunta" type="text" id="pregunta" value="<?php echo $_GET['pregunta'];?>" maxlength="100" onClick="this.value='';">
  ...
  <tr>
    ...
    <td><strong>Orden <a href="#"><img src="img/iconos/guardar.png" alt="Guardar orden" width="18" height="18" id="btnguardar" title="Guardar orden" onClick="MM_callJS('document.formguardar.MM_update.value=\'formguardar\';document.formguardar.submit();')"/></a></strong></td>
    <td><a href="faq_nuevo.php"><img src="img/iconos/nuevo.png" alt="Nuevo" width="32" height="32" title="Nuevo"/></a></td>
    </tr>
  <?php if ($totalRows_registros > 0) { // Show if recordset not empty ?>
    <?php do { ?>
      <tr>
        <td><?php echo $row_registros['pregunta']; ?></td>
        <td><input name="orden[]" type="text" id="orden[]" value="<?php echo $row_registros['orden']; ?>" size="3" maxlength="3">
          <input name="idfaq[]" type="hidden" id="idfaq[]" value="<?php echo $row_registros['idfaq']; ?>"></td>
        <td><a href="faq_editar.php?idfaq=<?php echo $row_registros['idfaq']; ?>"><img src="img/iconos/editar.png" alt="Editar" width="32" height="32" title="Editar"/></a>&nbsp;&nbsp;<a href="faq_borrar.php?idfaq=<?php echo $row_registros['idfaq']; ?>"><img src="img/iconos/borrar.png" width="32" height="32" alt=""/></a></td>
        </tr>
      <?php } while ($row_registros = mysql_fetch_assoc($registros)); ?>
    <?php } // Show if recordset not empty ?>
<tr>
  <td colspan="2"><?php if ($totalRows_registros > 0) { // Show if recordset not empty ?>
