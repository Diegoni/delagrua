...
<?php
if ((isset($_POST['idtaller'])) && ($_POST['idtaller'] != "")) {
  $deleteSQL = sprintf("DELETE FROM dlg_taller WHERE idtaller=%s",
  ...
  $deleteSQL = sprintf("DELETE FROM dlg_tallerservicio WHERE idtaller=%s",
                       GetSQLValueString($_GET['idtaller'], "int"));

  mysql_select_db($database_config, $config);
  $Result1 = mysql_query($deleteSQL, $config) or die(mysql_error());

  $deleteSQL = sprintf("DELETE FROM dlg_calificacion WHERE idtaller=%s",
                       GetSQLValueString($_GET['idtaller'], "int"));

  mysql_select_db($database_config, $config);
  $Result1 = mysql_query($deleteSQL, $config) or die(mysql_error());

  $deleteGoTo = "taller.php";
...
mysql_select_db($database_config, $config);
$query_registro = sprintf("SELECT idtaller, nombre FROM dlg_taller WHERE idtaller = %s", GetSQLValueString($colname_registro, "int"));
...
$colname_registro2 = "-1";
if (isset($_GET['idtaller'])) {
  $colname_registro2 = $_GET['idtaller'];
}
mysql_select_db($database_config, $config);
$query_registro2 = sprintf("SELECT idcalificacion FROM dlg_calificacion WHERE idtaller = %s", GetSQLValueString($colname_registro2, "int"));
...
<div class="uno"><a class="fa fa-wrench"></a> BORRAR TALLER</div>
<div class="entero">
  ...
  <table width="100%" border="0" cellspacing="0" cellpadding="4">
   <tr>
      <td colspan="4" align="center"><?php if($totalRows_registro2 > 0){ ?> El taller <?php echo $row_registro['nombre']; ?> tiene calificaciones asociadas.<?php  } ?> Se borrará el taller <?php echo $row_registro['nombre']; ?>, los servicios y califiaciones asociadas.       
