...
if (isset($_GET['nombreyapellido'])) {
  $colname_registros = $_GET['nombreyapellido'];
}
$colname2_registros = "";
if (isset($_GET['email'])) {
  $colname2_registros = $_GET['email'];
}
mysql_select_db($database_config, $config);
$query_registros = sprintf("SELECT idusuario, idgrupo, nombreyapellido, email, fblogin, activo FROM dlg_usuario WHERE nombreyapellido LIKE %s ", GetSQLValueString("%" . $colname_registros . "%", "text"));
if($colname2_registros != ""){
	$query_registros .= sprintf(" AND email LIKE %s ", GetSQLValueString("%" . $colname2_registros . "%", "text"));
}
$query_registros .= " ORDER BY email ASC";
...
<div class="uno"><a class="fa fa-wrench"></a> USUARIOS</div>
<div class="entero">
<table width="100%" border="0" cellspacing="0" cellpadding="4">
  <tr>
    <td colspan="6">
    <form action="usuario.php" method="get" id="formbuscar" name="formbuscar">
    <strong>Nombre y apellido
      <input name="nombreyapellido" type="text" id="nombreyapellido" value="<?php echo $_GET['nombreyapellido'];?>" maxlength="100" onClick="this.value='';"> 
      Email 
      <input name="email" type="text" id="email" value="<?php echo $_GET['email'];?>" maxlength="100" onClick="this.value='';">
  ...
  <tr>
    ...
    <td><strong>Sesión con facebook</strong></td>
    <td align="center"><a href="usuario_nuevo.php"><img src="img/iconos/nuevo.png" alt="Nuevo" width="32" height="32" title="Nuevo"/></a></td>
    </tr>
  <?php if ($totalRows_registros > 0) { // Show if recordset not empty ?>
    <?php do { ?>
      <tr>
        <td><?php echo $row_registros['nombreyapellido']; ?></td>
        <td><?php echo $row_registros['email']; ?></td>
        <td><?php if (!(strcmp($row_registros['idgrupo'],"1"))) {echo "administrador";} else {echo "registrado";} ?></td>
        <td><?php if (!(strcmp($row_registros['activo'],"1"))) {echo "si";} else {echo "no";} ?></td>
        <td><?php if (!(strcmp($row_registros['fblogin'],"1"))) {echo "si";} else {echo "no";} ?></td>
        <td align="center"><a href="usuario_editar.php?idusuario=<?php echo $row_registros['idusuario']; ?>"><img src="img/iconos/editar.png" alt="Editar" width="32" height="32" title="Editar"/></a>&nbsp;&nbsp;<a href="usuario_borrar.php?idusuario=<?php echo $row_registros['idusuario']; ?>"><img src="img/iconos/borrar.png" width="32" height="32" alt=""/></a></td>
      </tr>
