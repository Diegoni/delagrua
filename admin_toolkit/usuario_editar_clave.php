...
if ((isset($_POST["MM_update"])) && ($_POST["MM_update"] == "formguardar")) {
  if($_POST['claveguardada'] == md5($_POST['claveanterior'])){
	  $updateSQL = sprintf("UPDATE dlg_usuario SET clave=%s WHERE idusuario=%s",
						   GetSQLValueString(md5($_POST['clave']), "text"),
						   GetSQLValueString($_POST['idusuario'], "int"));
	
	  mysql_select_db($database_config, $config);
	  $Result1 = mysql_query($updateSQL, $config) or die(mysql_error());
	  $mensaje_ok = "La clave ha sido modificada.";
  } else {
	  $mensaje_error = "La clave no se modificó porque la clave anterior ingresada no es correcta.";
  }
}

$colname_registro = "-1";
if (isset($_SESSION['MM_Username'])) {
  $colname_registro = $_SESSION['MM_Username'];
}
mysql_select_db($database_config, $config);
$query_registro = sprintf("SELECT idusuario, email, clave, nombreyapellido FROM dlg_usuario WHERE email = %s", GetSQLValueString($colname_registro, "text"));
...
<div class="uno"><a class="fa fa-wrench"></a> EDITAR CLAVE</div>
<div class="entero">
<form method="POST" action="<?php echo $editFormAction; ?>" name="formguardar" id="formguardar" onSubmit="MM_validateForm('claveanterior','','R','clave','','R');return document.MM_returnValue">
  <table width="100%" border="0" cellspacing="0" cellpadding="4">
    <?php if($mensaje_ok != ''){ ?>
    <tr>
      <td colspan="4"><?php echo $mensaje_ok;?></td>
    </tr>
    <?php } else if($mensaje_error != ''){?>
    <tr>
      <td colspan="4"><?php echo $mensaje_error;?></td>
    </tr>
    <tr>
    <td><strong>Nombre y apellido</strong></td>
    <td><?php echo $row_registro['nombreyapellido']; ?>
    ...
    <td>&nbsp;</td>
      ...
      <tr>
        ...
        <td><input name="claveanterior" type="password" id="claveanterior" maxlength="50">
          <input name="claveguardada" type="hidden" id="claveguardada" value="<?php echo $row_registro['clave']; ?>"></td>
