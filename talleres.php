...
$currentPage = $url_relativa . "talleres/";
...
$colname_registros = "Vehiculo";
if (isset($_GET['tipovehiculo'])) {
  $colname_registros = $_GET['tipovehiculo'];
}
$colname2_registros = "Marca";
if (isset($_GET['marca'])) {
  $colname2_registros = $_GET['marca'];
}
$colname3_registros = "Provincia";
if (isset($_GET['provincia'])) {
  $colname3_registros = $_GET['provincia'];
}
$colname4_registros = "Localidad";
if (isset($_GET['localidad'])) {
  $colname4_registros = $_GET['localidad'];
}
$colname5_registros = "Servicio";
if (isset($_GET['servicio'])) {
  $colname5_registros = $_GET['servicio'];
}
...
mysql_select_db($database_config, $config);
$query_registros = "SELECT dlg_taller.idtaller, dlg_taller.nombre, dlg_taller.tipovehiculo, dlg_taller.tipovehiculo, dlg_taller.atencion, dlg_taller.precio, dlg_taller.servicio, dlg_taller.domicilio, dlg_localidad.localidad, dlg_provincia.provincia, dlg_marca.marca FROM dlg_taller LEFT JOIN dlg_marca ON dlg_taller.idmarca = dlg_marca.idmarca LEFT JOIN dlg_tallerservicio ON dlg_tallerservicio.idtaller = dlg_taller.idtaller LEFT JOIN dlg_localidad ON dlg_taller.idlocalidad = dlg_localidad.idlocalidad LEFT JOIN dlg_provincia ON dlg_taller.idprovincia = dlg_provincia.idprovincia LEFT JOIN dlg_servicio ON dlg_servicio.idservicio = dlg_tallerservicio.idservicio WHERE activo = 1 ";
if($colname_registros != "Vehiculo"){
	$query_registros .= sprintf(" AND dlg_taller.tipovehiculo LIKE %s ", GetSQLValueString($colname_registros, "text"));
}
if($colname2_registros != "Marca"){
	$query_registros .= sprintf(" AND dlg_marca.marca LIKE %s ", GetSQLValueString($colname2_registros, "text"));
}
if($colname3_registros != "Provincia"){
	$query_registros .= sprintf(" AND dlg_provincia.provincia LIKE %s ", GetSQLValueString($colname3_registros, "text"));
}
if($colname4_registros != "Localidad"){
	$query_registros .= sprintf(" AND dlg_localidad.localidad LIKE %s ", GetSQLValueString($colname4_registros, "text"));
}
if($colname5_registros != "Servicio"){
	$query_registros .= sprintf(" AND dlg_servicio.servicio LIKE %s ", GetSQLValueString($colname5_registros, "text"));
}
$query_registros .= " GROUP BY dlg_taller.idtaller ORDER BY dlg_taller." . $colname7_registros . " " . $colname6_registros;
...
<div class="izquierda screen"><!--izq-->
     ...
     <div class="navega-izq">
          <ul>
          <li><a href="<?php echo $url_relativa . "talleres/" . $_GET['tipovehiculo'] . "/" . $_GET['marca'] . "/" . $_GET['provincia'] . "/" . $_GET['localidad'] . "/" . $_GET['servicio'] . "/atencion/DESC/";?>">Mejor Atención</a></li>
          <li><a href="<?php echo $url_relativa . "talleres/" . $_GET['tipovehiculo'] . "/" . $_GET['marca'] . "/" . $_GET['provincia'] . "/" . $_GET['localidad'] . "/" . $_GET['servicio'] . "/precio/DESC/";?>">Mejor Precio</a></li>
          <li><a href="<?php echo $url_relativa . "talleres/" . $_GET['tipovehiculo'] . "/" . $_GET['marca'] . "/" . $_GET['provincia'] . "/" . $_GET['localidad'] . "/" . $_GET['servicio'] . "/servicio/DESC/";?>">Mejor Servicio</a></li>
          <li>Por nombre:<a href="<?php echo $url_relativa . "talleres/" . $_GET['tipovehiculo'] . "/" . $_GET['marca'] . "/" . $_GET['provincia'] . "/" . $_GET['localidad'] . "/" . $_GET['servicio'] . "/nombre/ASC/";?>"> A-Z</a> | <a href="<?php echo $url_relativa . "talleres/" . $_GET['tipovehiculo'] . "/" . $_GET['marca'] . "/" . $_GET['provincia'] . "/" . $_GET['localidad'] . "/" . $_GET['servicio'] . "/nombre/DESC/";?>">Z-A</a></li>
          </ul>
     </div><a name="buscar2"></a>
     <div class="cuadro-busq">
            <form id="formbuscar2" name="formbuscar2">
            <h1>NADA INTERESANTE?<br>BUSCA DE NUEVO!</h1>
            <li>
              <select name="tipovehiculo" id="tipovehiculo" onChange="MM_callJS('actualizar2();')">
                <option value="Vehiculo" <?php if (!(strcmp("Vehiculo", $_GET['tipovehiculo']))) {echo "selected=\"selected\"";} ?>>Vehiculo</option>
...
<option value="moto" <?php if (!(strcmp("moto", $_GET['tipovehiculo']))) {echo "selected=\"selected\"";} ?>>Moto</option>
              ...
              <input name="actualizacion" type="hidden" id="actualizacion">
            </li>
            <li>
              <select name="marca" id="marca" onChange="MM_callJS('actualizar2();')">
              <option value="Marca" <?php if (!(strcmp("Marca", $_GET['marca']))) {echo "selected=\"selected\"";} ?>>Marca</option>
                <?php
do {
?>
                <option value="<?php echo $row_registros9['marca']?>"<?php if (!(strcmp($row_registros9['marca'], $_GET['marca']))) {echo "selected=\"selected\"";} ?>><?php echo $row_registros9['marca']?></option>
                <?php
...
?>
            ...
            <li>
              <select name="provincia" id="provincia" onChange="MM_callJS('actualizar2();')">
                <option value="Provincia" <?php if (!(strcmp("Provincia", $_GET['provincia']))) {echo "selected=\"selected\"";} ?>>Provincia</option>
                <?php
do {
?>
                <option value="<?php echo $row_registros2['provincia']?>"<?php if (!(strcmp($row_registros2['provincia'], $_GET['provincia']))) {echo "selected=\"selected\"";} ?>><?php echo $row_registros2['provincia']?></option>
                <?php
...
?>
            ...
            <li>
              <select name="localidad" id="localidad" onChange="MM_callJS('actualizar2();')">
                <option value="Localidad">Localidad</option>
				<?php if($totalRows_registros3 > 0) {?>
				<?php
                do {
                ?>
                <option value="<?php echo $row_registros3['localidad']?>"<?php if(isset($_GET['localidad'])){  if (!(strcmp($row_registros3['localidad'], $_GET['localidad']))) {echo "selected=\"selected\"";} } else { if (!(strcmp($row_registros3['localidad'], $row_registro['localidad']))) {echo "selected=\"selected\"";} } ?>><?php echo $row_registros3['localidad']?></option>
                <?php
            ...
            <li><select name="servicio" id="servicio" onChange="MM_callJS('actualizar2();')">
              <?php if(isset($_GET['tipovehiculo']) && ($_GET['tipovehiculo'] == 'moto')) {?>
                <option value="Servicio" <?php if (!(strcmp("Servicio", $_GET['servicio']))) {echo "selected=\"selected\"";} ?>>Todos</option>
                <?php } else {?>
              <option value="Servicio" <?php if (!(strcmp("Servicio", $_GET['servicio']))) {echo "selected=\"selected\"";} ?>>Servicio</option>
              <?php
do {
?>
<option value="<?php echo $row_registros4['servicio']?>"<?php if (!(strcmp($row_registros4['servicio'], $_GET['servicio']))) {echo "selected=\"selected\"";} ?>><?php echo $row_registros4['servicio']?></option>
<?php
...
?>
            ...
            <li class="boton-b"><a href="#" onClick="MM_callJS('actualizar2();')">BUSCAR</a></li>
            </form>
            </div>
</div><!--izq-->
...
<div class="box-centro"><!--box resultado-->
          <h2><?php echo $row_registros['nombre']; ?> <a href="<?php echo $url_relativa . "taller/" . url2seo($row_registros['nombre']) . "-" . $row_registros['idtaller'] .  "/"; ?>" class="fa fa-plus-square"></a></h2>
          <h3>&nbsp;</h3>
...
<div class="izquierda mobile"><!--izq-->
    ...
    ?>
     <a name="buscar"></a>
     <div class="cuadro-busq">
            <form action="talleres.php" id="formbuscar" name="formbuscar" method="get">
            <h1>NADA INTERESANTE?<br>BUSCA DE NUEVO!</h1>
            <li>
              <select name="tipovehiculo" id="tipovehiculo" onChange="MM_callJS('actualizar();')">
                <option value="" <?php if (!(strcmp("Vehiculo", $_GET['tipovehiculo']))) {echo "selected=\"selected\"";} ?>>Vehiculo</option>
                <option value="auto" <?php if (!(strcmp("auto", $_GET['tipovehiculo']))) {echo "selected=\"selected\"";} ?>>Auto</option>
<option value="moto" <?php if (!(strcmp("moto", $_GET['tipovehiculo']))) {echo "selected=\"selected\"";} ?>>Moto</option>
              </select>
              <input name="actualizacion" type="hidden" id="actualizacion">
            ...
            <li>
              <select name="marca" id="marca" onChange="MM_callJS('actualizar();')">
                <option value="Marca" <?php if (!(strcmp("Marca", $_GET['marca']))) {echo "selected=\"selected\"";} ?>>Marca</option>
                <?php
do {
?>
...
?>
            ...
            <li>
              <select name="provincia" id="provincia" onChange="MM_callJS('actualizar();')">
                <option value="Provincia" <?php if (!(strcmp("Provincia", $_POST['provincia']))) {echo "selected=\"selected\"";} ?>>Provincia</option>
                <?php
do {
?>
                <option value="<?php echo $row_registros2['provincia']?>"<?php if (!(strcmp($row_registros2['provincia'], $_POST['provincia']))) {echo "selected=\"selected\"";} ?>><?php echo $row_registros2['provincia']?></option>
                <?php
} while ($row_registros2 = mysql_fetch_assoc($registros2));
  $rows = mysql_num_rows($registros2);
  if($rows > 0) {
...
?>
            ...
            <li>
              <select name="localidad" id="localidad" onChange="MM_callJS('actualizar();')">
            ...
            <li><select name="servicio" id="servicio" onChange="MM_callJS('actualizar();')">
              <?php if(isset($_GET['tipovehiculo']) && ($_GET['tipovehiculo'] == 'moto')) {?>
                <option value="Servicio" <?php if (!(strcmp("Servicio", $_GET['servicio']))) {echo "selected=\"selected\"";} ?>>Todos</option>
                <?php } else {?>
              <option value="Servicio" <?php if (!(strcmp("Servicio", $_GET['servicio']))) {echo "selected=\"selected\"";} ?>>Servicio</option>
...
do {
?>
<option value="<?php echo $row_registros4['idservicio']?>"<?php if (!(strcmp($row_registros4['servicio'], $_GET['servicio']))) {echo "selected=\"selected\"";} ?>><?php echo $row_registros4['servicio']?></option>
<?php
} while ($row_registros4 = mysql_fetch_assoc($registros4));
  $rows = mysql_num_rows($registros4);
  if($rows > 0) {
...
?>
            ...
            <input name="ordenarpor" type="hidden" id="ordenarpor" value="<?php if(isset($_GET['ordenarpor'])) { echo $_GET['ordenarpor'];} else { echo "nombre"; }?>">
            <input name="orden" type="hidden" id="orden" value="<?php if(isset($_GET['orden'])) { echo $_GET['orden'];} else { echo "ASC"; }?>">
            <li class="boton-b"><a href="#" onClick="MM_callJS('actualizar();')">BUSCAR</a></li>
            </form>
            </div>
</div><!--izq-->
...
<?php if ($totalRows_registros10 > 0) { // Show if recordset not empty ?>
  <?php do { ?>
   ...
   <div class="recientes">
      <!--recientes-->
      <h2><?php echo $row_registros10['nombre']; ?> <a href="<?php echo $url_relativa . "taller/" . url2seo($row_registros10['nombre']) . "-" . $row_registros10['idtaller'] . "/"; ?>" class="fa fa-plus-square"></a></h2>
